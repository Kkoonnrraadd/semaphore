...
function Apply-DatabaseTags {
    ...
    try {
        foreach ($tag in $Tags.PSObject.Properties) {
            ...
            # Apply each tag individually with retry logic
            $retryCount = 0
            $tagApplied = $false
            
            while (-not $tagApplied -and $retryCount -lt $maxRetries) {
                try {
                    ...
                    if ($retryCount -lt $maxRetries) {
                        Write-Host "    ⚠️  Tag application attempt $retryCount failed, retrying in $retryDelay seconds..." -ForegroundColor Yellow
        ...
        # Wait a few seconds for tags to propagate in Azure
        Write-Host "  ⏳ Waiting for tags to propagate..." -ForegroundColor Gray
        ...
        for ($i = 0; $i -lt $verifyRetries; $i++) {
            ...
            $allTagsPresent = $true
            foreach ($tag in $Tags.PSObject.Properties) {
                if (-not $currentTags.$($tag.Name) -or $currentTags.$($tag.Name) -ne $tag.Value) {
            ...
            if ($allTagsPresent) {
                ...
                if ($i -lt ($verifyRetries - 1)) {
                    Write-Host "    🔄 Tags not yet visible, waiting 10 seconds before retry $($i + 2)/$verifyRetries..." -ForegroundColor Yellow
        ...
        if ($tagsVerified) {
            Write-Host "  🏷️  ✅ Verified tags on $DatabaseName : $($tagList -join ', ')" -ForegroundColor Green
        }
        else {
            Write-Host "  🏷️  ⚠️  Applied tags to $DatabaseName but verification pending: $($tagList -join ', ')" -ForegroundColor Yellow
        }
    }
    catch {
